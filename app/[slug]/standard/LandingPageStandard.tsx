...
import { StandardSplashScreen } from './StandardSplashScreen';
import './standard.css';
...
export default function LandingPageStandard({ slugData }: { slugData: SlugData }) {
  const [screen, setScreen] = useState<Screen>('home');
  const [formPath, setFormPath] = useState<FormPath>('cicil');
  const [cicilEnter, setCicilEnter] = useState(false);
  const [crossfading, setCrossfading] = useState(false);
  /** Screen kept mounted underneath while the next screen fades in on top */
  ...
  return (
    <div className="relative mx-auto min-h-screen w-full max-w-md overflow-hidden bg-[#108EE9]">
      {/* Keep home mounted (no remount) so enter animations don't replay during fade */}
      {showHome && (
        <div
          className={
            underScreen === 'home'
