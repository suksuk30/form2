...
import { StandardV3HomeScreen } from './StandardV3HomeScreen';
...
export default function LandingPageStandardV3({ slugData }: { slugData: SlugData }) {
  ...
  const goToSplashFromInstan = useCallback(() => {
    setFormPath('cicil');
    crossfadeTo('splash');
  }, [crossfadeTo]);
  ...
  useLandingPhoneBack(screen, goHome, (target) => target === 'home', { historyKey: 'standardV3Screen' });
  ...
  const renderScreen = (target: Screen) => {
    switch (target) {
      case 'home':
        return (
          <StandardV3HomeScreen
            onActivateCicil={goToCicil}
            onActivateInstan={goToSplashFromInstan}
